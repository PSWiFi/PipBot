...
async function addPoints(user, room, amt, type = 0, cap = 10_000) {
  ...
  if (entry) {
    ...
    const pointsVal = Array.from({ length: type + 1 }, (_, i) =>
      i === type ? Math.min(cap, amt) : 0
    );
    const pointsDoc = new Points({
...
async function setPoints(user, room, setTo = 0) {
  return Points.findOneAndUpdate(
    { _id: `${toId(room)}-${toId(user)}` },
    { points: Array.isArray(setTo) ? setTo : [setTo] },
    { new: true }
  ).lean();
}
